...
plotMeanCIs(datCFZ=dat[dat$PCR.treatment=="CFZ",],datPLCB=dat[dat$PCR.treatment=="Placebo",],varName="ELISA.OD.norm",mTitle="",yLabel="Normalized OD",yLimits=c(-0.25,1),cexCust=2.25,lwd=4) # can't work on the log scale as ELISA data has actual zeroes in it

png(file=paste(sep="",outPrefix,"_Fig_ELISA-OD-OverTime.png"),height=9,width=16,units="in",res=450)
plotMeanCIs(datCFZ=dat[dat$PCR.treatment=="CFZ",],datPLCB=dat[dat$PCR.treatment=="Placebo",],varName="ELISA.OD.norm",mTitle="",yLabel="Normalized OD",yLimits=c(-0.25,1),cexCust=2.25,lwd=4)
dev.off()
...
datTmpCFZ$PCR.PATID<-factor(paste(sep="","CFZ",formatC(width = 2, format = "d", flag = "0",as.integer(factor(datTmpCFZ$PCR.PATID)))))

datTmpPlcb <- dat %>%
  filter(dat$PCR.treatment=="Placebo")
datTmpPlcb$PCR.PATID<-factor(datTmpPlcb$dummyPID)

datTmp<-rbind(datTmpCFZ,datTmpPlcb)
datTmp$PCR.PATID<-factor(c(as.character(datTmpCFZ$PCR.PATID),as.character(datTmpPlcb$PCR.PATID)))

for(pid in unique(datTmp$PCR.PATID[datTmp$PCR.treatment=="CFZ"])){
  g[[pid]]<-plotqPCRELISATimeHeatmap(datTmp,pid,ctThr=35)
}

for(pid in unique(datTmp$PCR.PATID[datTmp$PCR.treatment=="Placebo"])){
  g[[pid]]<-plotqPCRELISATimeHeatmap(datTmp,pid,ctThr=35)
}
...
Below is a colour legend for the heatmap above.

```{r, fig.width=9, fig.height=3, results='hide'}
plotDat<-datTmp %>% 
  mutate(PCR.log2_first.rs = scales::rescale(PCR.log2_first,to=c(0.2,1)),
         ELISA.OD.norm.rs = scales::rescale(ELISA.OD.norm,to=c(0.2,1))) %>%
  dplyr::select(PCR.PATID,PCR.Visit,PCR.treatment,PCRCT.Ct,PCRCT.Result,PCR.log2_first.rs,PCR.log2_first,ELISA.OD.norm.rs,ELISA.OD.norm,ELISA.Result)

legendHeatmapCFZPlcb<-function(pcrMin,pcrMax,elisaMin,elisaMax){
  palette <- c("qPCR\nlog2 oocyst", "ELISA\nnorm. OD")
  
  palCFZ<-colorRampPalette(colors=c(rgb(red=255,green=165,blue=0,alpha=0.2*255,maxColorValue=255),rgb(red=255,green=165,blue=0,alpha=255,maxColorValue=255)),alpha=T)(512L)
  palPlcb<-colorRampPalette(colors=c(rgb(red=70,green=130,blue=180,alpha=0.2*255,maxColorValue=255),rgb(red=70,green=130,blue=180,alpha=255,maxColorValue=255)),alpha=T)(512L)
  jcols<-cbind(palCFZ,palPlcb)
  
  maxlen <- 512L
  ncols <- ncol(jcols)
  lr.margin <- 25
  plot(0, 0, type = "n", xlab = "", ylab = "", xlim = c(-4*lr.margin, maxlen + 4 * lr.margin), ylim = c((ncols + 1) * 2-1, 1),yaxs = "i", xaxt = "n", yaxt = "n", xaxs = "i",bty="n")
  yseq <- seq(2, ncols * 2, length.out = ncols)
  axis(side = 2, at = yseq, palette, las = 1, lty=0, hadj = 0.5, cex.axis=1.5)
  for (c in 1:ncols) {
    pal <- jcols[, c]
    rect(xleft = lr.margin + seq(along = pal) - 0.5, ybottom = yseq[c] - 
           0.85, xright = lr.margin + seq(along = pal) + 0.5, 
         ytop = yseq[c] + 0.85, border = NA, lwd = 0, col = pal)
    if(c==1){
      text(x=lr.margin-2,y=yseq[c],labels=pcrMin,adj=c(1,0.5),cex=1.5)
      text(x=lr.margin+512+2,y=yseq[c],labels=pcrMax,adj=c(0,0.5),cex=1.5)
    }else if(c==2){
      text(x=lr.margin-2,y=yseq[c],labels=elisaMin,adj=c(1,0.5),cex=1.5)
      text(x=lr.margin+512+2,y=yseq[c],labels=elisaMax,adj=c(0,0.5),cex=1.5)
    }
  }
}

par(mar=c(0,5.1,0,2.1))
legendHeatmapCFZPlcb(pcrMin=format(nsmall=2,round(digits=2,min(plotDat$PCR.log2_first,na.rm=T))),
                     pcrMax=format(nsmall=2,round(digits=2,max(plotDat$PCR.log2_first,na.rm=T))),
                     elisaMin=format(nsmall=2,round(digits=2,min(plotDat$ELISA.OD.norm,na.rm=T))),
                     elisaMax=format(nsmall=2,round(digits=2,max(plotDat$ELISA.OD.norm,na.rm=T)))
                     )

png(file=paste(sep="",outPrefix,"_SupFig1_HeatMap_Ct35_legend.png"),height=3,width=9,units="in",res=450)
par(mar=c(0,5.1,0,2.1))
legendHeatmapCFZPlcb(pcrMin=format(nsmall=2,round(digits=2,min(plotDat$PCR.log2_first,na.rm=T))),
                     pcrMax=format(nsmall=2,round(digits=2,max(plotDat$PCR.log2_first,na.rm=T))),
                     elisaMin=format(nsmall=2,round(digits=2,min(plotDat$ELISA.OD.norm,na.rm=T))),
